...
#         print("Resetting any previously assigned frequent terms ...")
        ...
        for f in glob.iglob(os.path.join(os.getcwd(), 'LGM_Interlinking', config.default_data_path, f'*gram*_{encoding}.csv')):
...
#             print("Loading frequent terms from file {} ...".format(f))
            ...
            self.freq_ngrams[gram_type].update(
                pd.concat([df['term'].astype('str'), df['term'].astype('str').apply(lambda x: x[::-1])]).tolist())
