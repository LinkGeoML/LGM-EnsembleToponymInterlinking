...
# Load character list #
with open(os.path.join(os.getcwd(), "StringMatching/characters.pkl"), 'rb') as char:
...
class StringMatchingDataset(Dataset):
    ...
    def __init__(self, name):
        ...
        for dataset in train_datasets:
            self.data += list(csv.DictReader(open(os.path.join(os.getcwd(), 'data', dataset + '.csv')), delimiter=','))
