...
from StringMatching.layers import PenalizedTanh, HardAlignmentAttention
from StringMatching.utils import StringMatchingDataset, PadSequence
from StringMatching.variables import *
...
class Encoder(nn.Module):
    ...
    def forward(self, s, lengths):
        ...
        s_rep, last = self.gru2(s_rep, hidden_2)
        #s_rep, _ = torch.nn.utils.rnn.pad_packed_sequence(s_rep, batch_first=True)
        last = self.dropout(last)
        last = last.permute(1,0,2)
        last = last.contiguous()
        last = last.view(last.size()[0], -1) # Concatenate both directions
        return last

class RSModel(pl.LightningModule):
    ...
    def test_dataloader(self):
        return DataLoader(StringMatchingDataset(self.test_dataset), shuffle=False, num_workers=NUM_WORKERS, batch_size=BATCH_SIZE, collate_fn=PadSequence())
    ...
    def test_end(self, outputs):
        ...
        torch.save(self.state_dict(), os.path.join(os.getcwd(), 'StringMatching/weights/{}-{}.pt'.format(self.name, self.train_dataset)))
        ...
        for output in outputs:
            ...
            logits.append(output['out'])

        logits = np.vstack(logits)
        np.save(os.path.join(os.getcwd(), 'preds/rs_{}_preds.npy'.format(self.test_dataset)), logits)
